...
//IVREG2 analysis
ivreg2 cov_male (eduyears2=allele_score_21) [pweight=weight_hughes],cluster(mobi) endog(eduyears2)
regsave eduyears2 using "results/ivreg2_outcome_unadj",detail(all) pval replace
ds out_*
foreach i in `r(varlist)'{
	ivreg2 `i' (eduyears2=allele_score_21) [pweight=weight_hughes],cluster(mobi) endog(eduyears2)
	regsave eduyears2 using "results/ivreg2_outcome_unadj",detail(all) pval append
	}
	
*************************

use "results/ivreg2_outcome_unadj",clear
gen adjusted=0
...
matrix results_unadjusted=results'

mkmat coef stderr if adjusted==1 & cont==0,matrix(results) rownames(order)
matrix results_adjusted=results'

#delimit ;
coefplot (matrix(results_adjusted) , se(2) ms(T) msize(vsmall) mc(eltblue) ciopts(lc(eltblue) lwidth(vthin)) offset(+0.1))
		 (matrix(results_unadjusted) , se(2) ms(T) msize(vsmall) mc(edkblue) ciopts(lc(edkblue) lwidth(vthin)) offset(-0.1)) 
...
graph save "results/supplementary_figure_4_bin", replace;
#delimit cr
...
graph save "results/supplementary_figure_4_cont", replace 

graph combine "results/supplementary_figure_4_bin" "results/supplementary_figure_4_cont", col(1) imargin(0 0 0 0) ysize(8) xsize(6.26) graphregion(color(white))
graph export "results/supplementary_figure_4_adjusted_unadjusted.eps", as(pdf) replace fontface("Calibri")
...
save "results/unadj_results",replace
...
save "results/adj_results",replace



joinby depvar using  "results/unadj_results",
drop var order
